...
function Game() {
	...
	this.resetGame = function() {
		...
		$('#levelDescriptor').html("Level : " + this.level);
		setTimeout(function(){
			$('#newLevel').modal('show');
			self.setupLevel();
		}, 500);
	...
	this.updateCounts = function() {
		...
		$(".score").html("Clicked: <b>" + this.currentClicks +"</b>");
		$(".best").html("Best level: <b>" + this.bestLevel + "</b> (" + this.clicksForBest + " clicks)");
		$(".total").html("Total clicked: <b>" + this.totalClicks + "</b>");
	}
...
function GameBoard (wd, hi) {
	...
	this.getPadding = function(level) {
		if (level <= 6) return 15;
		if (level > 15) return 5;
	}
